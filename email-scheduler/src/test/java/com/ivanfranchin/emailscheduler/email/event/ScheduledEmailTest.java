...
class ScheduledEmailTest {
    ...
    void from_shouldCreateScheduledEmailWithCorrectFields() {
        CreateEmailRequest createEmailRequest = new CreateEmailRequest(
                "test@example.com", "Subject", "Body", 5000L
        );
        ...
        assertThat(scheduledEmail.expectedReturnTime()).isBetween(before.plusMillis(5000), after.plusMillis(5001));
    }

    @Test
    void from_nullSubjectAndBody_shouldDefaultToEmptyString() {
        CreateEmailRequest createEmailRequest = new CreateEmailRequest(
                "test@example.com", null, null, 1000L
        );
    ...
    void from_zeroDelay_shouldSetExpectedReturnTimeEqualToCreatedAt() {
        CreateEmailRequest createEmailRequest = new CreateEmailRequest(
                "test@example.com", "Subject", "Body", 0L
        );
