...
class CreateEmailRequestTest {
    ...
    void constructor_shouldCreateValidRequest() {
        CreateEmailRequest request = new CreateEmailRequest(
                "test@example.com", "Subject", "Body", 1000L
        );
    ...
    void validation_negativeDelay_shouldFail() {
        ...
        assertThat(violations).hasSize(1);
        assertThat(violations.iterator().next().getPropertyPath().toString()).isEqualTo("delayInMillis");
    }

    @Test
    void validation_nullDelay_shouldFail() {
        Validator validator = Validation.buildDefaultValidatorFactory().getValidator();
        CreateEmailRequest request = new CreateEmailRequest("test@example.com", "Subject", "Body", null);
